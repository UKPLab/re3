...
def main():
    ...
    task_name = 'review_request_extraction' # task_name in ['edit_intent_classification_M','edit_intent_classification_AD', 'revision_alignment','review_request_extraction']
    ...
    cot_type = 'L'  # select from ['LR','RL','L']
    icl_dynamic = ''  # select from ['','cat','diff','loc'], '' means no dynamic examples
    icl_dynamic_n = 3  # select from [1,3,5,8]
    icl_dynamic_file = None #Path(f'resource/RA_test_icl_roberta_cosine.pt')

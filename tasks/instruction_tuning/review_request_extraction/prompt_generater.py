...
def add_unique_id(data):
        data = data.fillna('')
        data['id'] = data['node_ix_src'] # the unique if for the review sentences
        return data
...
class PromptGenerater:
    ...
    def get_dynamic_examples(self, instance, icl_type):
        #in the icl_dynamic_file, the key is the unique id
        example_ixs = self.icl_dynamic_file[str(instance['id'])][icl_type][:self.icl_dynamic_n]
        train_df = self.train_ds.to_pandas()
        train_df = add_unique_id(train_df)
        dynamic_examples = train_df[train_df['id'].isin(example_ixs)]
        # order icl_examples by examples_ixs (most similar first)
        dynamic_examples = dynamic_examples.set_index('id').loc[example_ixs].reset_index()
